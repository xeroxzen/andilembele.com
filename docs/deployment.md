...
```text
/var/www/andilembele.com/
├── current -> releases/<git-sha>
└── releases/
    └── <git-sha>/
```

Nginx should use `/var/www/andilembele.com/current` as the document root. The existing TLS and server-block settings should remain unchanged.
...
install -d -o deploy -g deploy -m 755 /var/www/andilembele.com/releases
...
```nginx
root /var/www/andilembele.com/current;
```
...
| Variable | `DEPLOY_PATH` | `/var/www/andilembele.com` |
...
2. The deploy user can create a directory under `/var/www/andilembele.com/releases`.
3. Nginx points to `/var/www/andilembele.com/current`.
