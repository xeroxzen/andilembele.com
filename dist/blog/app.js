...
const remote=document.body.dataset.cms==='firebase';
const cloud=remote?await import('./firebase-client.js'):null;
const recoveryStorage=remote?sessionStorage:localStorage;
const recoveryKey=()=>remote?cloud.recoveryKey():'andile-editor-recovery';
const reading=p=>`${Math.max(1,Math.ceil(p.content.trim().split(/\s+/).length/200))} min read`;
async function api(url,options){if(remote&&url.startsWith('/api/'))return cloud.request(url,options);const r=await fetch(url,options);const data=await r.json();if(!r.ok)throw Error(data.error||'Request failed');return data;}
...
async function publicPage(){
 ...
 const slug=preview?'':new URLSearchParams(location.search).get('post')||location.pathname.split('/')[2];
...
async function editor(){document.title=(remote?'Editor':'Local editor')+' · Andile Jaden Mbele';document.querySelector('meta[name="description"]').content=remote?'Private writing workspace':'Local writing workspace';let token=(await api('/api/session')).token,originalSlug=null,dirty=false;
 app.innerHTML=`<section class="blog-head admin-heading"><div><p class="eyebrow">Editor</p><h1>Blog admin</h1></div><div class="admin-heading-actions"><a href="/blog/">View blog ↗</a></div></section><div class="cms-layout"><aside><div class="actions"><button id="new" type="button">New post</button><button id="backup" type="button">Export all posts</button>${remote?'<label class="file-button">Restore backup<input id="restore" type="file" accept="application/json,.json"></label>':''}</div><h2 class="small-heading">Your posts</h2><div id="post-list"></div></aside><section><form id="editor"><label>Title<input name="title" required maxlength="200"></label><div class="fields"><label>URL slug<input name="slug" required pattern="[a-z0-9]+(-[a-z0-9]+)*" maxlength="100"></label><label>Date<input name="date" type="date" required></label></div><label>Short description<textarea name="excerpt" rows="2" maxlength="1000"></textarea></label><label>Tags, separated by commas<input name="tags" placeholder="Software, Infrastructure"></label><div class="fields"><label>${remote?'Visibility':'Visibility on local blog'}<select name="status"><option value="draft">Draft (hidden)</option><option value="published">${remote?'Published':'Visible locally'}</option></select></label><label>Cover image<input name="coverImage" readonly placeholder="No cover selected"></label></div><div class="actions"><label class="file-button">Add image<input id="image" type="file" accept="image/png,image/jpeg,image/webp"></label><button id="clear-cover" type="button">Remove cover</button><label class="file-button">Import Markdown<input id="import" type="file" accept=".md,.txt,text/markdown,text/plain"></label></div><p class="help">An uploaded image becomes the cover. Its Markdown is also inserted at the cursor; remove it from the body if you only want a cover.</p><div class="toolbar">${Object.keys(blogSnippets).map(k=>`<button type="button" data-snippet="${k}">${k}</button>`).join('')}</div><label>Body<textarea name="content" required rows="18" maxlength="200000" spellcheck="true" placeholder="Write in Markdown…"></textarea></label><p class="help">Headings, bold, italic, links, lists, code, quotes, images, and lede/stat/source blocks. Raw HTML is shown as text.</p><div class="actions"><button class="primary" type="submit">${remote?'Save post':'Save post locally'}</button><button id="preview" type="button" aria-expanded="false">Preview</button><button id="download" type="button">Export Markdown</button><button id="delete-post" type="button" hidden>Delete post</button></div><p id="status" role="status" aria-live="polite"></p></form><section id="preview-body" class="article" hidden aria-label="Post preview"></section></section></div>`;
 if(remote){document.querySelector('#image').disabled=true;document.querySelector('#image').closest('label').hidden=true;document.querySelector('#clear-cover').hidden=true;document.querySelector('#image').closest('.actions').nextElementSibling.hidden=true;document.querySelector('[name=coverImage]').closest('label').hidden=true;}
 ...
 function stash(){dirty=true;try{recoveryStorage.setItem(recoveryKey(),JSON.stringify({post:post(),originalSlug}));status.textContent='Unsaved changes. Recovery copy stored in this browser.';}catch{status.textContent='Unsaved changes. Save to keep your work.';}if(!document.querySelector('#preview-body').hidden)renderPreview();}
 function populate(p,original=null){form.reset();for(const k of Object.keys(p))if(field(k))field(k).value=Array.isArray(p[k])?p[k].join(', '):p[k];originalSlug=original;document.querySelector('#delete-post').hidden=!original;dirty=false;document.querySelector('#preview-body').hidden=true;document.querySelector('#preview').setAttribute('aria-expanded','false');status.textContent='';}
 ...
 async function list(){const posts=await api('/api/admin/posts');const list=document.querySelector('#post-list');list.innerHTML=posts.length?posts.sort((a,b)=>b.date.localeCompare(a.date)).map(p=>`<button class="post-select" type="button" data-slug="${e(p.slug)}">${e(p.title)}<small>${p.status==='draft'?'Draft':remote?'Published':'Visible locally'} · ${e(p.date)}</small></button>`).join(''):'<p class="help">No saved posts yet.</p>';list.querySelectorAll('button').forEach(b=>b.onclick=()=>{if(dirty&&!confirm('Discard unsaved changes and open this post?'))return;populate(posts.find(p=>p.slug===b.dataset.slug),b.dataset.slug);recoveryStorage.removeItem(recoveryKey());});}
 ...
 form.onsubmit=async event=>{event.preventDefault();const button=form.querySelector('[type=submit]');button.disabled=true;try{const p=await api('/api/admin/posts',{method:'POST',headers:{'Content-Type':'application/json','X-CMS-Token':token},body:JSON.stringify({post:post(),originalSlug})});originalSlug=p.slug;document.querySelector('#delete-post').hidden=false;dirty=false;recoveryStorage.removeItem(recoveryKey());await list();status.textContent=p.status==='draft'?'Draft saved. Hidden from the public blog.':remote?'Published. Your post is now on the live blog.':'Saved and visible on the local blog. Nothing was deployed.';}catch(error){status.textContent=error.message;}finally{button.disabled=false;}};
 document.querySelector('#delete-post').onclick=async()=>{if(!originalSlug||!confirm('Permanently delete this saved post? Export it first if you need a backup.'))return;try{await api('/api/admin/posts/'+encodeURIComponent(originalSlug),{method:'DELETE',headers:{'X-CMS-Token':token}});populate(blank());recoveryStorage.removeItem(recoveryKey());await list();status.textContent='Post deleted.';}catch(error){status.textContent=error.message;}};
 document.querySelector('#new').onclick=()=>{if(dirty&&!confirm('Discard unsaved changes?'))return;populate(blank());delete field('slug').dataset.manual;recoveryStorage.removeItem(recoveryKey());};
 ...
 if(remote)document.querySelector('#restore').onchange=async event=>{
  const file=event.target.files[0];if(!file)return;
  try{
   if(file.size>20000000)throw Error('Choose a backup smaller than 20 MB.');
   if(!confirm('Restore these posts as private drafts? Existing posts will never be overwritten.'))return;
   const result=await api('/api/admin/restore',{method:'POST',body:await file.text()});await list();status.textContent=`Restored ${result.count} posts as drafts.`;
 ...
 try{const raw=recoveryStorage.getItem(recoveryKey());if(raw){const recovery=JSON.parse(raw);populate(recovery.post,recovery.originalSlug);dirty=true;status.textContent='Recovered unsaved work from this browser. Save to keep it.';}}catch{}
 await list();
}
function dirtyEditorConfirm(){return !recoveryStorage.getItem(recoveryKey())||confirm('Sign out and discard the unsaved recovery copy?');}
async function admin(){
 document.body.classList.add('admin-page');
 document.title='Admin · Andile Jaden Mbele';
 let session;
 try{session=await api('/api/session');}catch(error){
  if(remote){app.innerHTML='<section class="admin-login soft-card"><span class="login-mark" aria-hidden="true">↳</span><h1>Blog admin</h1><button class="primary" id="google-login">Continue with Google</button><p id="login-status" role="status"></p></section>';document.querySelector('#google-login').onclick=async()=>{const b=document.querySelector('#google-login');b.disabled=true;try{await cloud.login();await admin();}catch(e){document.querySelector('#login-status').textContent=e.message;b.disabled=false;}};return;}

  app.innerHTML='<section class="blog-head"><p class="eyebrow">Private workspace</p><h1>Sign in.</h1><form id="login"><label>Password<input name="password" type="password" autocomplete="current-password" required></label><button class="primary">Sign in</button><p id="login-status" role="status"></p></form></section>';
  document.querySelector('#login').onsubmit=async event=>{event.preventDefault();try{await api('/api/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({password:event.target.elements.password.value})});await admin();}catch(e){document.querySelector('#login-status').textContent=e.message;}};return;
 }
 await editor();
 if(remote||session.passwordProtected){
  const b=document.createElement('button');b.className='sign-out';b.innerHTML='<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true"><path d="M9 4H4v16h5M14 8l4 4-4 4M8 12h10"/></svg>Sign out';
  b.onclick=async()=>{if(dirtyEditorConfirm()){await api('/api/logout',{method:'POST',headers:{'X-CMS-Token':session.token}});location.href='/admin/';}};
  app.querySelector('.admin-heading-actions').append(b);
 }

}
try{if(location.pathname.startsWith('/admin')||location.pathname.replace(/\/$/,'')==='/blog/admin')await admin();else await publicPage();}catch(error){app.innerHTML=`<section class="blog-head"><h1>Unable to load.</h1><p>${e(error.message)}</p><a href="${location.pathname.replace(/\/$/,'')==='/blog/admin'?'/blog/admin':'/blog'}">Try again</a></section>`;}
