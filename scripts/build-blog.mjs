...
let shell=await read('dist/blog/index.html');
shell=shell.replace(/<main id="app">[\s\S]*?<\/main>/,`<main id="app">${indexMarkup(posts)}</main>`);
...
const preview=shell.replace('<body>','<body data-preview="true">').replace('<script type="module" src="/blog/app.js"></script>','').replace('<a href="/blog/admin">Editor</a>','').replaceAll('href="/writing"','href="/blog-preview.html"').replaceAll('href="/blog"','href="/blog-preview.html"').replace('</body>',`<script type="module">window.previewPosts=${JSON.stringify(posts).replaceAll('<','\\u003c')};\n${code.replaceAll('</script','<\\/script')}</script></body>`);
